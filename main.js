...
const make_current_keys_text = (k1, k2, n) => `Tap ${n <= 0 ? "?" : k1.toUpperCase()}/${n <= 1 ? "?" : k2.toUpperCase()} to gain pp.`;
// 1000000 -> 1,000,000
...
function make_shop_item(item, stat_text, item_type, click_action) {
    ...
    const p_stat = document.createElement("p");
    const item_text = document.createTextNode(`${item.name}, ${item.cost}${CURRENCY_TEXT}`);
    const stats = document.createTextNode(stat_text);
...
class Shop {
    constructor() {
        ...
        this.cps_upgrades_bought = new Array(this.cps_upgrades.length).fill(0);
        this.upgrades_bought = new Array(this.general_upgrades.length).fill(0);
    }
    get_multipliers() {
    ...
    buy(item) {
        ...
        const button = document.getElementById("button_item" + item.id);
    ...
    update_shop_element() {
        for (const i in this.upgrades_bought) {
            ...
            if (this.upgrades_bought[parseInt(i) - 1] > 0) {
                const li = document.getElementById("list_item" + item.id);
                li.hidden = false;
            }
        }
    }
    update_cps_shop_element() {
        ...
        for (const i in this.cps_upgrades_bought) {
            ...
            const button = document
                .getElementById("button_cps_item" + item.id);
            const stat = document
                .getElementById("p_stat_cps_item" + item.id);
            ...
            if (count > 0) {
                const producing = document
                    .getElementById("p_producing_cps_item" + item.id);
                const producing_text = `You have ${count}, producing ${(item.gives * count * multipliers[item.id]).toFixed(1)} ${CURRENCY_TEXT}/s`;
            ...
            if (this.cps_upgrades_bought[parseInt(i) - 1] > 0) {
                const li = document
                    .getElementById("list_cps_item" + item.id);
                li.hidden = false;
            }
        }
    }
